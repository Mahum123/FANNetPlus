...
for i in range(len(temp)):
    for j in range(4): #number of input features=4
...
for i in range(1,len(noiseCompleteSet)):
    if noiseCompleteSet[i-1]==0:
        ...
        noise.writelines([str(noiseCompleteSet[i-1][0]),", ",str(noiseCompleteSet[i-1][1]),", ",str(noiseCompleteSet[i-1][2]),", ",str(noiseCompleteSet[i-1][3]), ",\n"])
